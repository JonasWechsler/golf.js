...
CanvasCache.DEFAULT_CANVAS_WIDTH = 500;
...
var MarbleTexture = (function () {
    function MarbleTexture(WIDTH) {
        this.WIDTH = WIDTH;
        this.PERIOD = 4;
        this.COLOR0 = new Color(105, 112, 120);
...
var DungeonComponent = (function () {
    function DungeonComponent(width, height, left, top, cell_width, cell_height, start, rooms, adjacent, distance_from_start, main_path) {
        this.width = width;
        this.height = height;
        this.left = left;
        this.top = top;
...
var DungeonRenderSystem = (function () {
    ...
    DungeonRenderSystem.prototype.add_adjacent = function (dungeon, a, b) {
        dungeon.adjacent.at(a).push(b);
        dungeon.adjacent.at(b).push(a);
    };
    DungeonRenderSystem.prototype.adjacent_vectors = function (v) {
        return [
        ...
        for (var i = 0; i < dungeon.width; i++) {
            var _loop_2 = function (j) {
                ...
                if (self.open(dungeon, curr))
                    return "continue";
                this_2.adjacent_vectors(curr).forEach(function (adj) {
    ...
    DungeonRenderSystem.prototype.add_path = function (dungeon, start, length) {
        ...
        var _loop_3 = function () {
            ...
            var next = [];
            self.adjacent_vectors(curr).forEach(function (adj) {
    ...
    DungeonRenderSystem.prototype.openings = function (dungeon, i, j) {
        ...
        if (this.open(dungeon, v)) {
            this.adjacent_vectors(v).forEach(function (adj, idx) {
    ...
    DungeonRenderSystem.prototype.build_room = function (dungeon, i, j) {
        var v = new Vector(i, j);
        var TOP = 0, LEFT = 1, BOTTOM = 2, RIGHT = 3;
        var block_x = dungeon.left + i * dungeon.cell_width;
        var block_y = dungeon.left + j * dungeon.cell_height;
        var square_upper_left = new Vector(block_x, block_y);
        var square_lower_left = new Vector(block_x, block_y + dungeon.cell_height);
        var square_upper_right = new Vector(block_x + dungeon.cell_width, block_y);
        var square_lower_right = new Vector(block_x + dungeon.cell_width, block_y + dungeon.cell_height);
        var square_corners = [];
        ...
        for (var idx = 0; idx < 4; idx++) {
            if (open[idx]) {
                var adj = this.adjacent_vectors(v)[idx];
                var bdj = this.adjacent_vectors(v)[(idx + 5) % 4];
                var cdj = this.adjacent_vectors(v)[(idx + 3) % 4];
                var ccw_open = this.openings(dungeon, adj.x, adj.y)[(idx + 3) % 4] && open[(idx + 3) % 4];
                var cw_open = this.openings(dungeon, adj.x, adj.y)[(idx + 5) % 4] && open[(idx + 5) % 4];
    ...
    DungeonRenderSystem.prototype.render_walls = function (dungeon, x, y) {
    };
...
var DungeonGenerator = (function () {
    function DungeonGenerator() {
        this.adjacent = new VectorMap();
        this.main_path = [];
        this.distance_from_start = new VectorMap();
        this.rooms = new VectorMap();
    ...
    DungeonGenerator.generate = function () {
        ...
        var dungeon_component = new DungeonComponent(0, 0, 0, 0, 0, 0, undefined, undefined, undefined, undefined, undefined);
    ...
    DungeonGenerator.prototype.on_border = function (v) {
        return v.x == 0 || v.x == this.WIDTH - 1 ||
            v.y == 0 || v.y == this.HEIGHT - 1;
    };
    DungeonGenerator.prototype.in_bounds = function (v) {
        return v.x >= 0 && v.x < this.WIDTH &&
            v.y >= 0 && v.y < this.HEIGHT;
    };
    DungeonGenerator.prototype.open = function (v) {
        if (!this.in_bounds(v))
            return false;
        return this.adjacent.at(v).length == 0;
    };
    DungeonGenerator.prototype.adj = function (v) {
        return [
            new Vector(v.x, v.y - 1),
            new Vector(v.x - 1, v.y),
            new Vector(v.x, v.y + 1),
    ...
    DungeonGenerator.prototype.merge_adjacent = function (dist) {
        var self = this;
        for (var i = 0; i < self.WIDTH; i++) {
            var _loop_5 = function (j) {
                ...
                if (self.open(curr))
                    return "continue";
                this_4.adj(curr).forEach(function (adj) {
                    if (self.open(adj))
                        return;
                    if (!self.in_bounds(adj))
                        return;
                    var dist_curr = self.distance_from_start.at(curr);
                    var dist_adj = self.distance_from_start.at(adj);
                    if (Math.abs(dist_curr - dist_adj) < dist) {
    ...
    DungeonGenerator.prototype.add_path = function (start, length) {
        var self = this;
        var path = [start];
    ...
    DungeonGenerator.prototype.add_adjacent = function (next, parent) {
        this.adjacent.at(next).push(parent);
        this.adjacent.at(parent).push(next);
    };
    DungeonGenerator.prototype.find_rooms = function () {
        var room = 1;
        this.rooms = new VectorMap();
        var Q = [this.START];
        var _loop_7 = function () {
            var q = Q.shift();
            if (this_6.rooms.has(q))
                return "continue";
            this_6.rooms.map(q, room);
            var l = false, r = false, t = false, b = false;
            this_6.adjacent.at(q).forEach(function (adj) {
                if (adj.x == q.x - 1 && adj.y == q.y)
                    l = true;
                if (adj.x == q.x + 1 && adj.y == q.y)
                    r = true;
    ...
    DungeonGenerator.prototype.openings = function (i, j) {
        var _this = this;
        ...
        if (this.open(v)) {
            this.adj(v).forEach(function (adj, idx) {
                open[idx] = _this.open(adj) || !_this.in_bounds(adj);
            });
        }
        else {
    ...
    DungeonGenerator.prototype.build = function () {
        for (var i = 0; i < this.WIDTH; i++) {
            for (var j = 0; j < this.HEIGHT; j++) {
                var v = new Vector(i, j);
                var open_1 = this.openings(i, j);
                var TOP = 0, LEFT = 1, BOTTOM = 2, RIGHT = 3;
                var square_upper_left = new Vector(this.LEFT + i * this.CELL_W, this.TOP + j * this.CELL_H);
                var square_lower_left = new Vector(this.LEFT + i * this.CELL_W, this.TOP + (j + 1) * this.CELL_H);
                var square_upper_right = new Vector(this.LEFT + (i + 1) * this.CELL_W, this.TOP + j * this.CELL_H);
                var square_lower_right = new Vector(this.LEFT + (i + 1) * this.CELL_W, this.TOP + (j + 1) * this.CELL_H);
                var square_corners = [];
                square_corners[TOP] = square_upper_right;
                square_corners[LEFT] = square_upper_left;
                square_corners[BOTTOM] = square_lower_left;
                ...
                for (var idx = 0; idx < 4; idx++) {
                    if (open_1[idx]) {
                        ...
                        if (!(this.openings(adj.x, adj.y)[(idx + 3) % 4] && open_1[(idx + 3) % 4] && this.openings(cdj.x, cdj.y)[idx]))
                            add_line(openings[idx][0]);
                        if (!(this.openings(adj.x, adj.y)[(idx + 5) % 4] && open_1[(idx + 5) % 4] && this.openings(bdj.x, bdj.y)[idx]))
                            add_line(openings[idx][1]);
                    }
                    else {
                        add_line(walls[idx]);
...
function floor() {
    for (var i = 0; i < 5; i++) {
        var ent = new ECSEntity();
        var texture = new FloorTextureComponent(new MarbleTexture(8).generate());
...
function background() {
    ...
    view.x = view.y = -512 * 8;
    var ctx = view.content.getContext("2d");
    disableImageSmoothing(ctx);
...
var camera = new ECSEntity();
var ui_component = new UIComponent(0, 0, document.createElement("canvas"));
var camera_component = new CameraComponent();
camera_component.target = function () { return player.get_component(ComponentType.DynamicPhysics).position; };
camera.add_component(camera_component);
camera.add_component(ui_component);
var fps = new ECSEntity();
